...
rvsp_status_t rvsp_spgemm_work_estimation(rvsp_spgemm_descr_t descr,
  ...
  status = rvsp_symbolic_count(A->rows, A->row_ptr, A->col_idx, B->row_ptr,
                               B->col_idx, ws.mark, ws.touched, c_row_ptr,
                               op_counts, &total_nnz);
...
rvsp_status_t rvsp_spgemm_compute(rvsp_spgemm_descr_t descr,
  ...
  if (A->rows != descr->a_rows || A->cols != descr->a_cols ||
      A->nnz != descr->a_nnz || B->rows != descr->b_rows ||
      B->cols != descr->b_cols || B->nnz != descr->b_nnz) {
    return RVSP_ERROR_INVALID_ARGUMENT;
  }
  ...
  if (descr->filled_into != C->col_idx) {
    ...
    rvsp_symbolic_fill(descr->a_rows, descr->b_cols, A->row_ptr, A->col_idx,
                       B->row_ptr, B->col_idx, ws.mark, ws.scratch,
  ...
  if (descr->algo == RVSP_SPGEMM_ALGO_OMP) {
    memset(ws.acc, 0,
           (size_t)descr->b_cols * (size_t)omp_get_max_threads() *
               sizeof(float));
  } else
#endif
  {
  ...
  numeric(descr->a_rows, descr->b_cols, A->row_ptr, A->col_idx,
          (const float *)A->values, B->row_ptr, B->col_idx,
          (const float *)B->values, ws.acc, descr->c_row_ptr, C->col_idx,
          (float *)C->values);

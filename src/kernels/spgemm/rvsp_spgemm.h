...
/* O(nnz), no allocation. bad_row_out is optional and gets -1 if not row
 * specific. */
...
#define RVSP_SPGEMM_PARAMS                                                     \
  int32_t a_rows, int32_t a_cols, int32_t b_cols, const int32_t *a_row_ptr,    \
      const int32_t *a_col_idx, const float *a_values,                         \
      const int32_t *b_row_ptr, const int32_t *b_col_idx,                      \
      const float *b_values, void *workspace, size_t workspace_bytes,          \
      int32_t **c_row_ptr_out, int32_t **c_col_idx_out, float **c_values_out,  \
      int32_t *c_nnz_out, int64_t *op_counts_out
...
rvsp_status_t
rvsp_symbolic_count(int32_t a_rows, const int32_t *a_row_ptr,
...
#define RVSP_NUMERIC_PARAMS                                                    \
  int32_t a_rows, int32_t b_cols, const int32_t *a_row_ptr,                    \
      const int32_t *a_col_idx, const float *a_values,                         \
      const int32_t *b_row_ptr, const int32_t *b_col_idx,                      \
      const float *b_values, float *acc, const int32_t *c_row_ptr,             \
      const int32_t *c_col_idx, float *c_values

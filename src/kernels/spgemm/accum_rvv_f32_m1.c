 ...
 *
 * Gather-FMA-scatter accumulation at LMUL=1.
 */
...
#else
#error "accum_rvv_f32_m1.c requires the RISC-V V extension"
#endif
...
static inline void rvsp_accum_row(float a_val, int32_t b_nnz,
  ...
  while (p < b_nnz) {
    const size_t vl = __riscv_vsetvl_e32m1((size_t)(b_nnz - p));

    const vfloat32m1_t vb = __riscv_vle32_v_f32m1(&b_values[p], vl);

    const vint32m1_t vidx = __riscv_vle32_v_i32m1(&b_col_idx[p], vl);
    ...
    const vuint32m1_t voff =
        __riscv_vreinterpret_v_i32m1_u32m1(__riscv_vsll_vx_i32m1(vidx, 2, vl));

    vfloat32m1_t vacc = __riscv_vluxei32_v_f32m1(acc, voff, vl);

    vacc = __riscv_vfmacc_vf_f32m1(vacc, a_val, vb, vl);
...
#define RVSP_KERNEL_NAME rvsp_spgemm_rvv_f32_m1

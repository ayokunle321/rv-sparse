    ...
    float *RVSP_RESTRICT acc)
{
    ...
    {
        const size_t vl =
            __riscv_vsetvl_e32m1((size_t)(b_nnz - p));

        const vfloat32m1_t vb =
            __riscv_vle32_v_f32m1(&b_values[p], vl);

        const vint32m1_t vidx =
            __riscv_vle32_v_i32m1(&b_col_idx[p], vl);
        ...
        vfloat32m1_t vacc =
            __riscv_vluxei32_v_f32m1(acc, voff, vl);

 ...
 *
 * Gather-FMA-scatter accumulation at LMUL=2.
 */
...
#else
#error "accum_rvv_f32_m2.c requires the RISC-V V extension"
#endif
...
static inline void rvsp_accum_row(float a_val, int32_t b_nnz,
  ...
  while (p < b_nnz) {
    ...
    const vuint32m2_t voff =
        __riscv_vreinterpret_v_i32m2_u32m2(__riscv_vsll_vx_i32m2(vidx, 2, vl));
...
#define RVSP_KERNEL_NAME rvsp_spgemm_rvv_f32_m2

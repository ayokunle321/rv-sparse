...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    const int32_t *row_ptr = (const int32_t *)p.row_ptr->data;
    const int32_t *col_idx = (const int32_t *)p.col_ind->data;
    ...
    if (st == RVSP_CSR_OK) {
      snprintf(detail, sizeof(detail), "OK  (%s)", rvsp_csr_status_string(st));
    } else if (bad_row >= 0) {
      snprintf(detail, sizeof(detail), "FAIL %s (first bad row %d)",
               rvsp_csr_status_string(st), bad_row);
      failures++;
    } else {
      snprintf(detail, sizeof(detail), "FAIL %s", rvsp_csr_status_string(st));
      failures++;
    }

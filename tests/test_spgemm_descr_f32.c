...
int main(void) {
  ...
  check(rvsp_spgemm_set_algo(descr, RVSP_SPGEMM_ALGO_DEFAULT) == RVSP_SUCCESS,
...

  {
    ...
    int32_t ci[3];
    float v[3];
    rvsp_csr_matrix_t C = {2, 2, 3, rp, ci, v, RVSP_DTYPE_FP32, RVSP_FORMAT_CSR,
                           0};
    void *ws = malloc(4096);
  ...
  check(ops[0] == 2 && ops[1] == 1, "op counts match the fixture");
  check(rvsp_spgemm_get_op_counts(descr, ops, 1) == RVSP_ERROR_INVALID_ARGUMENT,
  ...
  check(rvsp_spgemm_compute(descr, &A, &B, &C, NULL) == RVSP_ERROR_NULL_POINTER,
        "compute rejects NULL workspace");
  check(rvsp_spgemm_compute(descr, &A, &B, NULL, ws) == RVSP_ERROR_NULL_POINTER,
